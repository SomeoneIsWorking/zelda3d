...
#
# Requires: source .env first (ZELDA3D_OOT3D_ROM); PIL (Pillow) for screenshots.
set -euo pipefail
...
do_boot() {
    ...
    if [[ -z "${ZELDA3D_OOT3D_ROM:-}" ]]; then
        echo "[oracle_boot] ERROR: ZELDA3D_OOT3D_ROM not set. Run: source .env" >&2
        exit 1
    fi
    if [[ ! -f "$ZELDA3D_OOT3D_ROM" ]]; then
        echo "[oracle_boot] ERROR: ROM not found: $ZELDA3D_OOT3D_ROM" >&2
        exit 1
    fi
