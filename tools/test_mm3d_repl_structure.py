...
class MmReplStructureTests(unittest.TestCase):
    ...
    def test_every_command_has_one_exact_production_match(self) -> None:
        expected = {
            "actors", "cam", "linkform", "linkinfo", "linkstate", "mlist", "mscale", "ping",
            "posinfo", "quit", "quitteardown", "roomwarp", "switchgame", "tp", "turn", "warp",
        }
        matches: list[str] = []
        for source in REPL.glob("*.c"):
            matches.extend(re.findall(r'Zelda3D_MmReplMatch\(command, "([a-z]+)"', source.read_text()))
        self.assertEqual(set(matches), expected)
        self.assertEqual(len(matches), len(expected), "a command has duplicate routing ownership")
    ...
    def test_router_and_entry_are_composition_only(self) -> None:
        ...
        forbidden = (
            "GET_PLAYER", "GET_ACTIVE_CAM", "Room_RequestNewRoom", "WindowRequest", "Entrance_IsValid",
            "Zelda3D_SetObjectScale", "actor.world", "transitionTrigger", "mkfifo", "open(", "read(",
        )
        for token in forbidden:
            self.assertNotIn(token, router, f"router absorbed domain implementation: {token}")
            self.assertNotIn(token, entry, f"entry absorbed domain implementation: {token}")
    ...
    def test_run_reset_composes_transport_and_persistent_framing(self) -> None:
        entry = (REPL / "mm3d_repl.c").read_text()
        reset = re.search(r"void Zelda3D_MmReplResetRunState\(void\) \{(?P<body>.*?)\n\}", entry, re.DOTALL)
