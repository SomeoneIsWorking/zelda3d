...
#
# Usage: tools/soh3d_hud_corruption_scan.sh [N]   (default 12)
set -u
...
repl(){ tools/soh3d_repl.py cmd "$1" 2>/dev/null; }

for i in $(seq 1 "$N"); do
  ii=$(printf "%02d" "$i")
  echo "=== launch $ii/$N ==="
  tools/soh3d_game.sh stop >/dev/null 2>&1
  # COLD: each launch is a fresh process (the race lives at boot/texture-upload).
  SOH3D_HEADLESS=1 SOH3D_AUTO=1 SOH3D_LINK=1 tools/soh3d_game.sh start "$ENTR" "$TIME" >/dev/null 2>&1
  ...
  tools/soh3d_repl.py shot "hudscan/launch_$ii" >/dev/null 2>&1
...
echo "=== detector ==="
shopt -s nullglob
python3 tools/soh3d_hud_detect.py "$OUT"/launch_*.png
