...
bool LoadRepoEnvironment() {
    ...
    for (char** entry = environ; entry && *entry; ++entry) {
        ...
        if (equals) callerNames.emplace(*entry, static_cast<size_t>(equals - *entry));
    }
    ...
    if (readFailed || status == -1 || !WIFEXITED(status) || WEXITSTATUS(status) != 0) {
        std::fprintf(stderr,
                     "harness: environment %s failed to evaluate (status=%d, bytes=%zu)\n",
                     envPath.c_str(), status, output.size());
        return false;
    }
...
int main(int argc, char** argv) {
    if (!LoadRepoEnvironment()) return EXIT_FAILURE;
