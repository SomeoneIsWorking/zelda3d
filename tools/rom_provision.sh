...
#   1. an already-set environment variable (ZELDA3D_3DS_ROM / ZELDA3D_N64_ROM)
...
#
# - ZELDA3D_3DS_ROM (decrypted OoT3D .3ds): required by Zelda3D asset rendering.
# - ZELDA3D_N64_ROM (.z64/.n64/.v64): only needed the first time, to extract oot.o2r.
...
zelda3d_provision_roms() {
    ...
    if [ -z "${ZELDA3D_3DS_ROM:-}" ]; then
        ZELDA3D_3DS_ROM="$(_zelda3d_first_file "$repo/oot3d.3ds" "$repo"/*.3ds || true)"
    fi
    if [ -z "${ZELDA3D_N64_ROM:-}" ]; then
        ZELDA3D_N64_ROM="$(_zelda3d_first_file "$repo/oot.z64" "$repo"/*.z64 "$repo"/*.n64 "$repo"/*.v64 || true)"
    fi

    [ -n "${ZELDA3D_3DS_ROM:-}" ] && export ZELDA3D_3DS_ROM
    [ -n "${ZELDA3D_N64_ROM:-}" ] && export ZELDA3D_N64_ROM
    ...
    if [ -n "$sohdir" ] && [ -n "${ZELDA3D_N64_ROM:-}" ] && ! _zelda3d_have_n64_assets "$sohdir"; then
        ln -sf "$ZELDA3D_N64_ROM" "$sohdir/$(basename "$ZELDA3D_N64_ROM")" 2>/dev/null || true
    fi
}
