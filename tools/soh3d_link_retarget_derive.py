...
The "replace rotation" retarget (SoH3D_UpdateAnimN64Mapped: OoT3D bone local rot := N64
...
Usage:
  . ./.env && python3 tools/soh3d_link_retarget_derive.py scratch/bin/link_joints.csv
"""
import sys, os, re
import numpy as np
...
ANIMMAP = "Shipwright/soh/src/soh3d/soh3d_player_animmap.inc"

# OoT3D childlink_v2 bone -> N64 player limb (must mirror kLinkChildBoneMap in soh3d.c).
...
def main():
    ...
    for bid in range(25):
        ...
        lines.append(f"    {{ {limb:3d}, {mode}, {{ {flat} }} }}, // b{bid}")
        if limb >= 0:
            print(f"  b{bid:2d}<-limb{limb:2d}: mode={mode} "
                  f"({'rest' if mode==0 else 'replace' if mode==1 else 'left C·R' if mode==2 else 'right R·C'})")
    with open(out_inc, "w") as f:
        f.write("// GENERATED by tools/soh3d_link_retarget_derive.py — do not edit by hand.\n")
        f.write("// Per OoT3D childlink_v2 bone: { n64Limb, mode, C[9] } where mode 0=rest, 1=replace\n")
        f.write("// (local rot := N64 jointTable rot), 2=left (C·R_n64), 3=right (R_n64·C). C is a\n")
        f.write("// row-major 3x3 constant rest-frame correction; identity unless mode>=2. Legs/neck\n")
        f.write("// stay on pure replace (verified, no regression); divergent spine/arms get C.\n")
        f.write("// Derived QUANTITATIVELY from the live N64 jointTable vs the OoT3D CSAB (the best\n")
        f.write("// proxy for the authored pose); residual floor ~0.4 is irreducible authoring noise.\n")
        f.write("static const SoH3dBoneCorr kLinkChildBoneCorr[25] = {\n")
        f.write("\n".join(lines))
        f.write("\n};\n")
    print(f"\nwrote {out_inc}")
