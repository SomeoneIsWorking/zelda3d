...
def main():
    ...
    # Emit a SEED reference (NOT the game file). The game's Shipwright/.../soh3d_bonemap.inc is the
    ...
    # they are never clobbered. Lines here are ready to paste into kSoH3dBoneMaps.
    seed_path = os.path.join(SK, 'soh3d_bonemap.seed.inc')
    with open(seed_path, 'w') as f:
        f.write("// SEED (reference only) from tools/soh3d_skel_export.py — paste/adjust entries into\n")
        f.write("// the hand-maintained Shipwright/soh/src/soh3d/soh3d_bonemap.inc. NOT compiled.\n")
        for zar in sorted(bonemap):
            e = bonemap[zar]
            n = e['oot3d_bone_count']
            arr = [str(e['bone_to_limb'].get(str(i), -1)) for i in range(n)]
            f.write('    SOH3D_BONEMAP("%s", %.5ff, %d, %s), // %s\n' % (zar, e['scale_ratio'], n, ', '.join(arr), e['object']))
    print("seed:", len(bonemap), "-> tools/skeldata/soh3d_bonemap.seed.inc (master .inc is hand-maintained)")
