...
Drives Link through every ported force-state via the `linkstate` REPL (mm/2s2h/Z3DRepl.c ->
mm/src/.../z_player.c Zelda3D_PlayerForce* hooks) and validates each one: the hook must install
its expected action func (or, for a context-gated state with no live context, decline SAFELY)
and MM must survive. This is the mm_sweep.py orchestrator's state-coverage half (re_control_debug_
backlog #11); the OoT3D/MM3D-oracle parity comparison is a later add-on (blocked on the MM oracle).
...
Usage:
    tools/mm_game.sh start          # or let this script's --start do it
...
import mm_control  # reuse its REPL_FIFO + _reply plumbing
...
STATES = [
    ...
    ("carry",    "Player_UpperAction_CarryActor", ["NO heldActor"]), # needs a lifted actor
    ("climb",    "Player_Action_50",           []),                  # reply always names it; "no wallPoly"/"declined" are OK sub-states
...
def q(cmd, settle=0.4):
    return mm_control._reply(mm_control.REPL_FIFO, cmd, settle=settle)


def mm_alive():
    r = subprocess.run([os.path.join(REPO, "tools", "mm_game.sh"), "status"],
                       capture_output=True, text=True)
    return "instances: 0" not in r.stdout
...
def main(argv):
    if "--start" in argv:
        subprocess.run([os.path.join(REPO, "tools", "mm_game.sh"), "stop"],
                       capture_output=True)
        time.sleep(2)
        subprocess.Popen([os.path.join(REPO, "tools", "mm_game.sh"), "start"],
                         stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        time.sleep(14)

    if not mm_alive():
        sys.exit("MM not running — start it (tools/mm_game.sh start) or pass --start.")
    ...
    for state, install_tok, decline_toks in STATES:
        ...
        if not alive:
            ...
            verdict = "DECLINE"   # context-gated, safe — expected without the live context
        else:
            verdict = "UNEXPECTED"
            fails.append(state)
    ...
    if fails:
        ...
        return 1
    print(f"PASS: all {len(STATES)} force-states install cleanly (or safely decline); MM survived.")
    return 0
