#!/usr/bin/env bash
# #16 first-person early-load crash repro loop. Boots SoH3D on the REAL GPU directly into a scene
# with SOH3D_FP_REPRO=1 (engine synthesizes C-up edges at the first controllable frame so the
...
#
#   tools/soh3d_fp_repro_loop.sh [entrance] [attempts] [seconds_per_boot]
...
for i in $(seq 1 "$ATTEMPTS"); do
    ...
    sleep 1
    SOH3D_FP_REPRO=1 timeout "$SECS" "$REPO/tools/soh3d_gpu_launch.sh" "$ENTR" >"$LOG" 2>&1
    rc=$?
    pkill -9 -f soh.elf 2>/dev/null
