...
| `core/` | ⬜ pending (Phase 2b) | Not started. |
| `render/` | ⬜ pending (Phase 2b) | Not started — see `repl/` row: the REPL extraction attempt showed render + REPL share ~29 file-static symbols, so these two are effectively one combined extraction, not independent. |
| `repl/` | 🟡 attempted, deferred (Phase 2) | **Assessed, not landed.** `Zelda3D_ReplExec` (~2,226 lines of `strcmp` command dispatch) + `Zelda3D_ReplPoll` (~280 lines) sit as the literal tail of `zelda3d.c` (L3807-EOF pre-extraction), so a dependency scan is exact: 29 file-`static` symbols declared earlier in `zelda3d.c` are referenced from that tail — `sModelTable` (30 refs total), `sAuto` (16), `Zelda3D_EmitModelDraw` (7), `Zelda3D_AutoMode` (12), `Zelda3D_ActiveSkyIndex` (4), `Zelda3D_UpdateLight` (4), `Zelda3D_ReconcileCutsceneCam`, `Zelda3D_ActorObjectId`, `Zelda3D_FogSetPosition`, `Zelda3D_InitForceTime`, `Zelda3D_N64FloorCb`, `Zelda3D_SkyModelId`, plus the generic-actor-control statics (`sZelda3dActorPin{Pos,Rot}`, `sZelda3dSelDraw{Model,Scale,GroundOff,DsHave,DsLiftY,DsLocal}`, `sZelda3dMotion{File,Actor,Remaining,Frame}`, `gZelda3dAim{Center,Radius}`, `gZelda3dEnKoMaskOverride{,Set}`, `sWarpPlay`). All are used elsewhere in the (not-yet-extracted) render/camera/model-dispatch code too — this is genuinely the SAME tangle as the deferred `render/` extraction, not a REPL-only dependency set. Extracting REPL cleanly requires un-static'ing + header-exposing this whole render-state surface, i.e. doing it well means doing it together with `render/` (Phase 2b), not as an isolated low-risk move like `scene/`/`hud/` were. Left entirely in `zelda3d.c` rather than force a shim over 29 symbols with no build to verify against. `Zelda3D_ReplReply` (small, already `extern`-declared in `zelda3d.h` for `zelda3d_link.cpp`) is the one piece that WOULD move cleanly on its own if a future session wants a small first cut. |
