...
| **Player / Link (draw + pose + 3DS behavior)** | 🟡 draw done; 14 OoT3D divergences ported (2026-07-22), 8 blocked on engine-side RE | `player/zelda3d_link.cpp/.h`, **`player/zelda3d_sword_trail.{cpp,h}`** (new), `behaviors/actor/player.h`, `tables/zelda3d_link_bonecorr.inc`, `tables/zelda3d_player_animmap.inc`; ported deltas also live in vendored `z_player.c` / `z_player_lib.c` / `z_camera.c` / `z_parameter.c` / `z_play.c` / `sys_math3d.c` | `debug_journal/2026-07-22-oot3d-player-port-campaign.md`, `oot3d-decomp/docs/{player_port,divergence_map}.md`, `docs/link_parity_checklist.md` | **A wholesale z_player.cpp port is the WRONG shape** — 730 OoT3D functions swept and every Link bug chased to ground was byte-exact to N64; Grezzo did not rewrite Link. The real payload is the catalogued DIVERGENCES. A 65-agent sweep turned 41 of them into specs: **14 ready (ALL PORTED)**, 10 rejected by adversarial verification (42% of implementable ones), 10 not divergences at all, 8 blocked. Ported: buoyancy 2/3 rescale, Iron-Boots vetoes (talk/A-press), Zora-tunic boot regs, first-person entry pose, two camera SFX deletions, sword-trail tip trim, jump sword-clank, Lens obtainability, plane epsilon, dead-input gate, guarded Master Sword regive, in-water item buttons. **`divergence_map.md` is a HYPOTHESIS, not a work list** — 5 rows named the wrong function outright and were corrected. Verification: smoke test passes; jump/roll/attack/swim_surface/swim_dive all MATCH the oracle — but swim was MATCH before the buoyancy change too, so that is no-regression, NOT proof of improved fidelity. |
