...
| **Kanban / backlog** | ✅ done | `KANBAN.md` (local markdown board — edit directly), `BACKLOG.md` (pointer) | `CLAUDE.md` kanban section | Local board IS the source of truth (GitHub Issues + `tools/kanban.py` dropped 2026-07-17). USER-DRIVEN work only; sweep-discovered gaps must NOT become cards (fix in-session + journal instead, per hard rule). |
...
| **MM3D asset/format decomp + rigid render** | 🟡 rigid LIVE (2026-07-17); skinned gated | `Shipwright/cmb3d/asset/{gar,lzs}.{h,cpp}` (GAR2 parser + LzS inflate), `2ship/2s2h/zelda3d/mm3d_model.cpp` (objectId→GAR2→CMB→draw + SkelAnime hooks) | memory `mm3d-assets-gar2`; `docs/re-frontier.md` `mm3d.gar2-parser` = re-verified | **No longer a blocker.** GAR2 parser + LzS inflate exist, verified on the real ROM (raw + LzS archives). **Verified LIVE in-game 2026-07-17** (`tools/mm_game.sh`, East Clock Town): rigid MM3D actors load + render their 3DS CMBs (run log `[MM3D] loaded model .../zelda2_tokei_turret.gar.lzs`, `tokei_tobira` mapped rigid). Remaining gap: SKINNED actors are gated OFF by default (`ZELDA3D_MM_SKINNED_TPOSE`). NOTE (2026-07-17 code read): Stage 2 (SkelAnime retarget) IS wired — skinned actors defer in `Zelda3D_TryDrawActor`→`SetPending`, then the SkelAnime hook (`mm3d_draw.c:131`) runs `mmUpdateAnimN64` (live N64 joints, Rz·Ry·Rx) + emit; it's not stuck at Stage-1 bind-pose. It's gated because pose quality with the **identity bone→limb map** isn't ship-ready — `mm3d_model.cpp:377` flags a per-archive bone-map for rigs whose CMB bone order diverges from N64 limb order as the fix. Concrete next step: frame a complex skinned NPC with the gate on to grade the identity retarget vs. where a bone-map is needed. |
...
| `model/` | ✅ Phase 3 done | `zelda3d_model.cpp/.h`, `zelda3d_cmab.cpp/.h`, `zelda3d_overlay2d.cpp/.h`, `zelda3d_model_internal.h` moved via `git mv`; every relative include across `zelda3d/` (and stale comment-paths in `mm3d_model.cpp`) fixed to match. |
