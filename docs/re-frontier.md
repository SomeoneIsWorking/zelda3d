...
- status: re-partial
- deps: render.multi-stage-tev
- evidence: `oot3d-decomp/docs/fragment_lighting.md`; candidate `CmbRenderer` vtable slots `FUN_003f9b5c` / `FUN_003fa5d0`; cache-owned 99-draw Save-overlay negative that reaches neither slot; cache-owned Kokiri negatives for recovered model-submission helper `FUN_004c7ab0` and direct virtual PICA material-state route `FUN_003fbba8` → `FUN_003fb5ec` → `FUN_003fb9ac`; `debug_journal/2026-08-30-cmb-fragment-lighting-disabled.md`; offline corpus survey (1,997 CMBs / 11,172 materials, zero failures); Dark Link retail close-test
