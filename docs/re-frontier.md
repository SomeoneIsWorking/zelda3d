...
- gap: MECHANISM UNDERSTOOD; the SWORD-specific part is one locate away. Reading N64 Player_DrawImpl (z_player_lib.c:1051) beside the 3DS body gave an exact correspondence and settled both data questions (oot3d-decomp/docs/player_draw_impl_located.md, commit 2431064): the mask/shift tables at 0x0053cbc4/0x0053cb1c are gUpgradeMasks/gUpgradeShifts (NOT equip tables — N64's upgrade tables already have 8 entries with these exact values, including the irregular 12->14 step because the wallet field is 2 bits), so index 2 is UPG_STRENGTH and gSaveContext+0xb8 is inventory.upgrades; and the pair table at 0x0053c74c is sBootDListGroups indexed boots-1 with exactly TWO valid rows, (35,36) and (15,22), so param_4 is the boots argument. IMPORTANT: that block is gauntlet plates, boot pieces and the child's Goron bracelet — it has NOTHING to do with Link's sword, so porting it would NOT fix kanban #201 e and must not be shipped as that fix. The sword/sheath-on-back visibility lives in the LIMB-DRAW OVERRIDE (N64 Player_OverrideLimbDrawGameplayDefault), whose 3DS twin is a sibling of the two override addresses already known — 0x004c5378 (FirstPerson) and 0x004c5214 (Crawling) — and should be cheap to locate from the same Player_Draw call site. NEXT: (1) locate that default-override twin and RE its sheath/sword mesh selection; (2) port it into a dedicated zelda3d module replacing the hand-curated Zelda3D_LinkComputeMidMask and verify on the full user path; (3) separately, the gauntlet/boots/bracelet selection is now fully RE'd and is a cheap independent port.
- notes: 
