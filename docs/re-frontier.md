...
- evidence: `tools/mm_phase_tour.py` is the CLI over focused session/artifact/orchestration/catalog/report owners using shipping runtime `ZELDA3D_MM_PHASE_REPORT=1`; offline tests cover successful reports, zero denominators, unmapped animations, static clips with enough per-actor samples, and thin multi-actor aggregates.
...
- gap: The 12-scene serial tour has not been run on the changed tree, so it supplies no new live coverage yet. Run it after a fresh Clang build; require exact scene IDs, a nonzero final denominator, zero unmapped animations, and no static pair with at least two samples per actor. A clean result widens evidence only to those twelve scenes and does not by itself justify default-on or close the PARTIAL-15 policy.
- notes: This is a driver and validator, not a second animation implementation: phase/morph sampling stays in shipping code. Artifacts remain under gitignored `scratch/mm_phase_tour/`, and the single-instance lock serializes the game.
...
- status: re-partial
- deps: mm.skinned-csab
- evidence: `mm3d-decomp/docs/player_models.md` inventories the retail form archives and explicit CMB identities: Fierce Deity `zelda2_link_boy_new/link_demon`, Goron `zelda2_link_goron_new/link_goron`, Zora `zelda2_link_zora_new/link_zora`, Deku `zelda2_link_nuts_new/link_deknuts`, and Human `zelda2_link_child_new/link_child`. Focused policy/contract tests exercise the exact form mapping, explicit CMB selection, pending-draw handoff, and Player's `SkelAnime_DrawFlexLod` interception under Clang.
- where: Form policy and adapter: `2ship/2s2h/zelda3d/mm3d_player_model_policy.{cpp,h}` and `mm3d_player_model.{cpp,h}`; draw orchestration: `mm3d_player.{c,h}`, explicit catalog route in `mm3d_model_catalog.{cpp,h}`, and the Player LOD seam in `2ship/src/code/z_skelanime.c`.
- gap: The shared `/actors/zelda2_link_new.gar.lzs` animation bank contains 847 CSABs but no body CMB; animation selection therefore still needs a form-directory-aware route from that shared bank. Equipment/hand mesh visibility and a live rendered form tour are also unverified, so `MM_ZELDA3D_LINK` remains opt-in.
- notes: Player's stock draw still supplies the live skeleton, joint table, override/post callbacks, and their side effects. The LOD seam consumes the pending MM3D body, then re-walks the N64 skeleton under the existing collider/side-effect guard while restoring display-list pointers. Generic object-short-name lookup is not an authority for Player because the retail bodies live in form-specific `_new` archives.
...
- status: re-partial
- deps:
- evidence: `oot3d-decomp/docs/boss_fd2.md`, `build/decomp/{00365860,003696ec,0036b96c,003c724c}.c`, and ARM disassembly establish the 30 Hz movement/history producer, unit authored-tick integration, interpolated trig table, and title-owned atan2 polynomial. The 2026-08-26 embedded-oracle forced-profile run reached exact zero-delta through 270 authored ticks for producer position/rotation/velocity/speed/turn/move timer and the sampled 150-entry history. The positive control changed one oracle history X value by 1000 and produced `DIVERGED` (`meanPos=50`, `maxPos=1000`); restoring it returned exact `MATCH`. Both `libsoh_core.so` and `soh3d_harness` were linked under Clang for that run.
- where: Shipping orchestration: `Shipwright/soh/src/zelda3d/behaviors/actor/boss_fd.{h,cpp}`; authored movement/history, forced flight/death, effect override, shared profile, and history layout: `behaviors/actor/boss_fd/`; lifecycle/dispatch: `behaviors/actor_behavior.{h,cpp}` and the narrow `code/z_actor.c` bridge. Harness: `tools/soh3d_harness/{actor_layout.h,actor_compare.*,boss_fd_compare.*,soh_boss_fd_state.cpp}`.
- gap: The exact producer verdict covers only the constant forced action-0 profile. General action/death production remains partial because the recovered steering → action → tail order cannot be reconstructed from only pre/post host endpoints. A paired user-facing rendered-image run is still required before declaring the visible flying body at parity.
- notes: Authored state is per actor through typed `ObjectExtension`, not a process singleton. `ActorBehavior::preUpdate` runs immediately before `actor->update`; integer phase `+=3`, ticks `phase/2`, remainder `phase%2` emits one then two authored ticks per 20 Hz host updates without floating drift. The forced profile keeps the N64 overlay's raw fly-speed input at 5 while verifying the authored `FUN_003C724C` control/writeback at 10/3. Stop-clear and timer zero are initialization conditions, not predicate fields. The validated comparator distinguishes `MATCH`, `DIVERGED`, `MISSING`, and `INVALID` and reports paired producer velocity before ring warm-up.
