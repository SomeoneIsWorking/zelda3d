...
- gap: The exact shared-bank route and base mesh reset have live five-form submission and phase evidence. The sheath/back-shield selector passed its focused and shared Clang gates and a live authentic Human equipment transition: Kokiri sword + Hero shield submitted `0x370000028` at idle, held R drove the normal shield/model path to `0x370000020`, and release restored `0x370000028`. Both hand selectors and the bottle material constant are grounded and ported; the bottle route now has authentic live held/put-away mesh and two-state material proof. Authentic live sword/instrument capture and a Deku-spin material capture remain open. The left-hand owner deliberately leaves two unresolved retail details inactive: MM3D-private Player+`0x129bc` bit 16 during mount/contact and retail-only Zora wait/demo callback identities. Full visual/retarget parity does not follow: generic framing placed four forms partly below the proof images, Deku normal wait/walk resources were unmapped, and one short-tour Human walk-end-right pair stayed at frame zero. The 20 unnamed `gameplay_keep_Linkanim_*` resources and non-`gPlayerAnim_` identities deliberately remain unmapped, so `MM_ZELDA3D_LINK` remains opt-in.
