...
- gap: CORRECTED DIAGNOSIS (claim C007, replacing C006). There is NO red divergence — red is ~zero on BOTH sides, as it should be for this water layer. The texture is essentially at parity (0.96/0.89/0.87). What is wrong is the vertex-lit PRIMARY: ours is +42% in G and +35% in B, and the combiner output follows at +45%/+37%. Note this does NOT contradict C004 (lighting/material/fog/combiner ruled out): C004 ruled out the UNIFORM STATE, which d9 and d11 share byte-identical, and d11 is at parity — so with the same uniforms and the same lighting math, a PRIMARY that is 40% high for d9 alone points at the PER-VERTEX COLOUR DATA for this mesh, not at the lighting computation. NEXT: read the water material's vertex colours out of the CMB at the data level (Shipwright/cmb3d) and compare against what the oracle's vertex lighting implies — with matDif=(0,0,0,1), dif0=0 and matAmb=(1,1,1,0) x amb0=(0.42745,0.42745,0.48627), oracle PRIMARY ~ vtxColour x ambient, so oracle vtxColour_GB ~ (69.7/109, 84.5/124) and ours would be ~40% above that. Suspect a scale/normalisation on the colour attribute (a 4-bit or 5-bit component expanded wrongly, or a missing divide) rather than component order, since red is correct at zero on both sides. SECOND, SMALLER THREAD: explain the 0.4005 source factor — it is NOT the shader's frag.a (mode 8 forces 1.0), so it comes from the pipeline blend config; if it differs from what the oracle applies, the magnitudes above shift.
