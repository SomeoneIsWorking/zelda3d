...
- gap: (1) the 3DS DRAW-side 0.64 site not pinned (undecompiled 0.64f pools VA 0x254ac4/0x279748/0x325a20; value double-sourced N64+3DS-root-motion literals); (2) anim-movement (movementFlags 0x9B) root-motion consumption not mirrored in the CSAB draw — possible hip-x/z double-apply during door-open/time-travel anims, needs a live EnDoor pass-through repro FIRST; (3) ~~real ladder-grab climb never engaged headless~~ **CLOSED 2026-07-23** — `tools/ladder_repro.py` grabs and climbs deterministically (see `player.anim-map-coverage`); climb-entry anchor now live-verified, no jump; (4) idle stands +0.94 units above actor.y (authentic per the 0.64 literal vs bind ratio 0.6095) — confirm with one matched-camera oracle A/B.
