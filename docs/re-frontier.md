...
- gap: RE DONE for the mechanism (oot3d-decomp/docs/actor_shadow.md, commit 7d82eb4) — the PORT is what remains. ActorShadow_DrawFeet @ 0x001d04f4 is a FAITHFUL port of N64 (same structure, same constants), so nothing changes there; the whole difference is ActorShadow @ 0x0033e450, which Grezzo REWROTE. It builds and caches a 3D model object (asset 0x51) per shadow slot and draws it with grey = 1 - min(1, lightWeight*5e-5) * shadowAlpha/255 applied MULTIPLICATIVELY (reaches pure black at full weight, where an alpha-blended texture never does), yaw = atan256(light->dir.x, light->dir.z) * PI/128, stretch = max(1, 4.5 - light->dir.y * 0.035), and the matrix chain floorMtx -> translate(0,1.5,0) -> rotateY(yaw) -> scale(s,1,s*stretch). All constants read from code.bin, none guessed. That accounts for both halves of the measured gap: darker (multiplicative) and much longer (up to 4.5x elongation along the light azimuth) — the large soft region to Link's lower-left in scratch/screenshots/oracle_kday.png is that stretch, not terrain shading. PORT PLAN: reproduce the transform + colour math in the SoH ActorShadow path. The identity of asset 0x51 (the shadow model) is still open, so a first pass can use our existing shadow texture and still capture the rotation, the stretch and the multiplicative darkening.
- notes: 
