...
- notes: **MECHANISM RESOLVED 2026-07-17 (journal §5d)** — hypothesis (a) root motion CONFIRMED, (b) cue-window-decode RULED OUT. The SoH twin `EnHorse_CsWarpRearing` (cue 0x41 → funcIdx 5) forces ENHORSE_ANIM_REARING→IDLE in-place with speedXZ=0 (can't translate), but the real FUN_002535f0 zeroes speed AND dynamically SELECTS a clip via +0x1b0 (FUN_0036ae14/FUN_003204a4 anim-change). Root-motion probe (§5c): rearing/idle/gallop are in-place, but slowrun2_30/transitions carry net root Z. Oracle horse translates → it plays a net-root-motion clip, not rearing → SoH's "rearing→idle" is an UNFAITHFUL port of FUN_002535f0's anim selection. FIX: 0x41 handler must play the +0x1b0 clip and consume its root-bone translation onto world.pos (not integrate the zero speed). Remaining unknown = exact +0x1b0 clip id (needs dynamic obs / Ghidra of the non-rider-struct +0x1b0 writer), but mechanism + fix shape settled.
...
- gap: **divergence NOT localized — the earlier "at.y term is the SOLE divergence" conclusion was FALSIFIED (2026-07-17).** The Camera_CalcAtDefault extra term `at.y += player[0x1760]·−0.01` (3DS FUN_00338ac8 vs SoH z_camera.c:929) is **real but MOTION-ONLY**: `player[0x1760]` decays 400/frame to 0 and clears its enable flag (0x100) whenever Link is not actively rising in the walk/run action (writer FUN_00250ad0 else-branch, 00250ad0.c:1186-1205), so at the *matched/idle* pose the 28-unit drift was measured under (normal1.h, commit 28f24f23) the term is exactly 0 and at.y is IDENTICAL on both sides. A zero term cannot cause a persistent offset. Since `eye.y = at.y + r·sin(pitch)` and at.y matches at idle, the persistent drift — IF it is a real steady state and not a non-converged-spring / init-path artifact — is in the **eye distance (r) or pitch** path (FUN_00239fd8 param_1[0x51]/[0x52] springs + the FUN_00367df4 at→eye VecSph, vs SoH eyeAdjustment.r + Camera_CalcDefaultPitch). What DOES match and is ruled out: yOffset formula (identical), both pitch clamps (0x38A4/−0x3C8C), LERP rates (0.1/0.2). **NEXT is EMPIRICAL, not more static RE:** build SoH, warp to Kakariko, hold matched Link pose to spring convergence, compare `posinfo` eye.y vs the oracle's camera eye.y; confirm the drift is a real steady state before diffing eye r/pitch. RE gains that stand from the at.y detour: action `0x4ba378` = ground walk/run locomotion; `player[0x2c]` = world.pos.y; `player[0x10c]` = world.pos.y snapshot at state transitions; decay 400/frame, threshold 9. (The motion-only at.y Y-bias is a separate, real 3DS behavior worth porting later — but it is NOT this drift.) Do NOT approximate with a magic −25 or port a decay-to-zero term to "fix" an idle offset. **MEASURED (2026-07-17):** SoH-side converged Kakariko (scene 0x52, entrance 0xDB, NORM1 live funcIdx=2, Link idle at converged pose): Link=(-2451,138,1062) yaw=16384 → at=(-2451,**182**,1062), eye=(-2627,**214**,1062); i.e. at.y = Link.y+44 (yOffset), eye.y = at.y+32. `Zelda3D_ReconcileCutsceneCam` (the 18.0f CamLift) is cutscene-only (returns 0 for MAIN_CAM/csCtx-idle) so it is OFF here — this is pure NORM1 output. **BLOCKED on the oracle side:** the Azahar binary is MISSING (`Azahar/build/bin/Release/azahar` absent; only `build-libretro` exists), so `cam_status` at the matched Kakariko pose can't be read — the oracle A/B needs Azahar built first. That build is the concrete unblock for confirming whether eye.y=214 diverges from OoT3D (real steady-state r/pitch drift) or matches (28-unit was a non-converged/init artifact → close camera.normal1).
- notes: **NOT a cheap flip** (assessed 2026-07-16): raw Ghidra body 3152 bytes, ~15 unnamed helpers, ~30 DAT_ constants, param_1[N] float indices. PROGRESS 2026-07-17 (journal `2026-07-17-oot3d-dat-constants...`): (1) **DAT_ constants are now readable** — code.bin @ VA−0x100000 ([[soh3d-oot3d-dat-constants]]), base confirmed by DAT_0023a34c=68.0 matching SoH's 68.0f; the "~30 unresolved constants" blocker is GONE. (2) **yOffset formula RULED OUT as the divergence** — FUN_00239fd8's height/yOffset (lines 84-86, with a354=0.01/a358=1.0/a34c=68.0) is ALGEBRAICALLY IDENTICAL to SoH Camera_Normal1's yNormal·PCT(h) (z_camera.c:1593-94). So the 28-unit Δeye-Y is in the EYE/SWING path, not yOffset. Next: map FUN_00239fd8's later eye/pitch writes (param_1[0x43..0x52]) against SoH's eyeAdjustment/swing block (z_camera.c:1660+), diff to localize the delta, port ONLY that (over SoH's already-faithful N64 body). Offset anchors: [0x20]=at [0x23]=eye [0x29]=eyeNext? [0x35]=play [0x36]=player [0x6c]=speedXZ. Still do NOT guess — resolve helpers/offsets first; use `cammode` for the A/B.
- notes: 
