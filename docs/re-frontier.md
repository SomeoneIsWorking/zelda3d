...
- gap: FIX WRITTEN, BUILDS, NOT YET VERIFIED — deliberately left UNCOMMITTED in the working tree (Shipwright/soh/src/zelda3d/player/zelda3d_link.cpp, LinkMidMask::compute child sheath switch). The rule implemented, ported from OoT3D 0x004c70c4 and N64 z_player_lib.c:1420-1441, is TWO rules not one: (a) SHEATH_18/19 — if the child has no Kokiri sword on B AND currentShield < PLAYER_SHIELD_HYLIAN, use the (child, no sword) row: Deku shield alone (LINK_MID(13)) or bare; Hylian/Mirror are excluded so a child carrying one keeps normal back geometry; (b) SHEATH_16/17 — no Kokiri sword means nothing on the back at all. VERIFICATION IS THE BLOCKER, and the naive approach does not work: setting bitem also changes the HUD's B-button icon, and the scene-title card animates, so a whole-frame diff between the two states reads ~32k changed pixels across the entire screen and proves nothing. A crop of Link's back was also inconclusive because the camera framing shifted between captures. WHAT TO DO NEXT: freeze the actor and camera (afreeze 1 + camfreeze) so the two captures are pixel-identical apart from the back geometry, capture bitem 0x3B vs 0xFF, and diff ONLY the back region; expect the sword hilt to vanish while the Deku shield stays. Tooling for the state now exists: REPL  (commit ed84020a, verified reading 0x3b and writing 0xff). The stronger check afterwards is the real user path — a save before the Kokiri sword is collected — rather than a forced field.
- notes: 
