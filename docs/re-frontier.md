...
- gap: Ground-form emergence, idle/turn, fire/claw, burrow, hit→vulnerable, damage, and death CSAB transitions are recovered and ported with independent authored playheads plus OoT3D five-frame crossfades. Every retail persistent action now has one exact initial-CSAB policy; an unknown action invalidates the controller and retains the native draw instead of inventing `vba_wait`. Body/fire-hair CMABs now use the recovered actor-local 60-authored-fps clock, the exposed-face pulse resets/ticks from typed `BossFd::faceExposed`, and the shared fire-hair CMB is submitted unlit as authored. A paired live capture proves the local clocks reach the renderer but does not close the stable body-wide brightness residual. The former rendered-head blocker is resolved, but full parity remains open: exercise the natural live sequence with matched action timing, distinguish the remaining deterministic solver residual from moving-root/trig precision, and reverse-engineer the named OoT3D mechanism behind the remaining body/material brightness difference rather than fitting a gain. Flying Boss_Fd is tracked separately by render.boss-fd-flying.
