...
- status: in-progress
- deps:
- evidence: Recovered OoT3D movement/history ground truth: `oot3d-decomp/docs/boss_fd2.md`, `build/decomp/00365860.c`, and functions `FUN_0036B96C`/`FUN_003C724C` establish raw speed/turn integration, unscaled collision displacement, and a 30 Hz history producer. Existing live controls/screenshots still cover earlier multipart/material/death/effect branches. The stabilized source now has a pre-update authored producer plus an extracted non-mutating harness comparator. The shipping BossFd translation units compile under Clang, but no current shipping or harness link and no comparator/runtime run exist.
...
- gap: Linked builds restored 2026-08-25 and the paired comparator RUNS live (drive recipe: scratch/bossfd_compare_drive.py; oracle+shipping warp 0x305, force bossfd_profile, run-90+soh_step-30 paired slices). Cadence parity verified: dMove=0 at warm end (oracle moveTimer +0.5/frame, authored +1.5/host-update). Ground-truth fix landed: forced kSpeed was the N64 overlay's 5.0; the genuine FUN_003C724C writes control := pool(0x003c76d0) = 10/3 every tick — profile and authored writeback now use 10/3. dSpeed=dTurn=0 verified. REMAINING: baseline compare is DIVERGED with quantified rotation drift (meanRot≈1.8 rad over the ring) despite matching approach constants/semantics — residual localized to FUN_003C724C's yaw/pitch target-derivation block (decomp lines ~60–95, Ghidra C lossy there); needs disasm-level recovery before a MATCH baseline can exist, after which the fault-injection positive test (force bossfd_fault requires preceding MATCH) still must run to certify the instrument. The forced profile does not cover the general action/death producer.
- notes: Authored state is per actor through typed `ObjectExtension`, not a process singleton. `ActorBehavior::preUpdate` runs immediately before `actor->update`; integer phase `+=3`, ticks `phase/2`, remainder `phase%2` emits one then two authored ticks per 20 Hz host updates without floating drift. The shared profile predicate checks action 0, target `(0,500,300)`, speed 5, turn/max 1000, and wobble amplitude 20/rate 0. Stop-clear and timer zero are `fdfly` initialization conditions, not predicate fields; the comparator separately pairs scene/action/current authored timer. General action/death remains partial because the recovered steering -> action -> tail order cannot be reconstructed from only pre/post host endpoints. `MATCH`, `DIVERGED`, `MISSING`, and `INVALID` remain designed verdicts, not yet trusted instrument evidence.
