...
- gap: UNBLOCKED — the polarity question is RESOLVED (claim C010, oot3d-decomp commit 9215377): flag 1 = DRAWN, and the base loop in 0x004c4560 is the RESET stage of a per-frame rebuild, not the final state. Verified from the Show/Hide pair on the same array (0x0037266c writes 1, 0x0036932c writes 0) plus the first-person tail block that HideMeshes 0x1b-0x30 indices. ARCHITECTURE TO PORT: every draw, reset all mesh flags to 0 except the three age-specific core meshes (adult {45,46,47}, child {24,25,26} from the table at 0x004dc388), then explicitly enable per age/gear/state via the rest of 0x004c4560, its helpers 0x004c71dc and 0x004c70c4, and Player_DrawImpl's equipment block. That is WHY a curated mask can never be right — the real system is a reset plus enable rules, not a list. REMAINING: decode the id tables at 0x0053c5a8 / 0x0053c6ec and the per-instance table at player[0x1bc]; RE helpers 0x004c71dc and 0x004c70c4 fully; identify selector fields 0x1b4/0x1b5 (hand types), 0x1ac, 0x1a6, 0x1749, 0x121c, 0x1710, 0x6c, 0x29b8 (first-person bit); then port into a dedicated zelda3d module replacing Zelda3D_LinkComputeMidMask and verify on the full user path — a Link with no sword must not show one on his back.
- notes: 
