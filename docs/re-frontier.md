...
- gap: THE SWORD SELECTOR IS FOUND: helper 0x004c70c4 (oot3d-decomp commit 426efd5). It decodes CUR_EQUIP_VALUE(EQUIP_TYPE_SWORD) — ldrh gSaveContext+0x8a (equips.equipment) & gEquipMasks[0]=0x000F (table 0x0053cb0c) >> gEquipShifts[0]=0 (table 0x0053cb08, a BYTE array {0,4,8,12}) — and selects a mesh from one of two 12-u32 tables at 0x0053c4b8 and 0x0053c4d8, read as six adult/child pairs. Those tables contain -1 entries, which is SetMeshVisible's no-op sentinel, in exactly the rows where no sword should be drawn. That is precisely what the hand-curated Zelda3D_LinkComputeMidMask lacks and why #201 e shows a sword before Link picks one up. TWO TRAPS RECORDED IN THE DOC: (1) the shift table is BYTES — Ghidra typed it short and rendered the read as a shift by 1024, the ARM is ldrb; (2) these equip tables are NOT the ones at 0x0053cbc4/0x0053cb1c, which are gUpgradeMasks/gUpgradeShifts for UPG_STRENGTH — two similar pairs sit a few dozen bytes apart and swapping them attaches strength rules to the sword. REMAINING: pin what indexes the six rows (six rows for four sword values suggests a sword-value-plus-slot composite); establish which table is the back-worn sheath and which the held blade; name player[0x1b6] (0x12/0x13 branch), [0x1c4], [0x1a6], gSaveContext[0x80], and the 0x4000 bit of player[0x29b8]; then port into a dedicated zelda3d module replacing Zelda3D_LinkComputeMidMask and verify on the full user path.
- notes: 
