...
- gap: THE OLD BLOCKER WAS A FALSE PREMISE — this row said the fn-ptr table at 0x4bff48 is reached via base+offset so Ghidra finds zero refs, and called it MULTI-SESSION. There is no table: 0x4bff34..0x4bff5c is a literal pool embedded in Player_Draw's tail (its floats 1.05/0.95/1.1 are N64 Player_Draw's Matrix_Scale), and 0x4bff48 is read by a plain ldr r2,[pc,#0x278] at 0x004bfcc8. FindRangeRefs saw nothing because Ghidra had not disassembled that region AS CODE — an instrument failure, not a property of the binary. The scan that works: look in code.bin for ldr rX,[pc,#imm] whose computed target (va+8+/-imm) is the address you want. REMAINING, in order: (1) RE func_0x002b9bf8 at 0x002b9bf8 — confirm it is set-mesh-visible-by-id and what its third argument means; (2) resolve the iRam004c16xx pool anchors and read the id table at iRam004c16a0 plus the mask/shift feeding the index from player[0xb8]; (3) map OoT3D mesh ids to our CMB mesh_id values; (4) port into a zelda3d module replacing the hand-curated Zelda3D_LinkComputeMidMask, and verify on the FULL user path — a Link with no sword must not show one on his back (kanban #201 e). The equipment-less oracle save still blocks a live A/B, so steps 1-2 stay static.
- notes: 
