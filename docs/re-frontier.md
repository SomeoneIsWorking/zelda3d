...
- notes: **NARROWED 2026-07-22 to the VERTEX COLOUR** (`debug_journal/2026-07-22-kokiri-near-terrain-overbright-is-vertex-colour.md`). Per-stage A/B against the oracle's own per-fragment TEV probe (new `SOH3D_HARNESS_SW=1` software-rasterizer mode + `SOH3D_PIXEL_TEX`, whole-draw dump): texture 1.04/1.02 (parity), PRIMARY_COLOR **1.18/1.17**, final 1.18/1.18; our live light term measures (1.4195,1.4194,1.2547) = exactly 2x the scene ambient the oracle reports, so the excess is the per-vertex colour we feed that material (~1.24/1.23/1.08 vs the 3DS's). **Two suspects FALSIFIED by direct A/B, do not retry:** (a) the synthetic mip chain — built with it disabled (mipLevels=1 + max_lod=0), d8 moved 1.184 -> 1.181; (b) the ported PICA fog — REPL `fog3d 0` moves d8 1.184 -> 1.180 while correctly driving the far draws (d9 1.033->0.799, d7 0.979->0.617), and our fog window is byte-identical to the oracle's. **CONFIRMED, not merely A/B-fitted:** the `clamp(lightSum * vColor)` product-clamp form — at Kokiri the sum is 1.4196>1, yet the oracle's PRIMARY scales LINEARLY with the ambient across dayTime 0x6000->0x4000 (77,77,69)->(51,51,26). NEXT: RE what the 3DS's CmbVShader feeds as the colour attribute for `spot04_0_info.zsi`'s terrain material (our group g3, first=2661 count=2727, matched to oracle d8 by nv=2727) vs what `cmb.cpp` decodes; the oracle's attribute scales are `vtxScl0=(1,0.007874,0.0039216,0)`. The gap is hue-shaped (R/G ~1.24, B ~1.08), not a single scale factor.
