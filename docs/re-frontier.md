...
- gap: needs a scene/state that ACTUALLY sets bit 0x100 (climb/wall/pull/fast-rise) to observe a real divergence and A/B against — do NOT port speculatively off Kakariko-idle where it's inert. Remaining field maps: `player[0x10c]` snapshot timing (captured at state transitions, e.g. 003b055c.c:502), the accumulate scale, and the FUN_00359690 predicate. Do NOT approximate with a magic −25 constant.
