...
- status: in-progress
- deps:
- evidence: oot3d-decomp/docs/boss_fd2.md; embedded-oracle 600-sample emergence→idle→fire trace (`scanned=600`, five controller transitions) and live actor matrix; headless natural-camera `scratch/screenshots/fd2_controller_proc_f88.png`; SKELSCALE model 2005 ratio=1.000 scale=0.007; shipping `zelda3d_app` Clang build and `boss_fd2_animation_policy_test` pass. On 2026-08-27 the new shipping `fd2info` diagnostic observed `vba_search`, `vba_hit`, `vba_beforedamage`, and `vba_damage` advancing through typed live state transitions. A fresh model-2004 on/off pair then isolated a visible 57-pixel-wide vertical body silhouette, falsifying the same day's earlier “no visible body” note. The paired harness command `force bossfd2_ground` now drives the recovered child `+0x124` → parent `+0x940` handoff in both engines and accepted live oracle/SoH actor pairs. Generic long B×3 then A input dismisses the oracle heat warning. The harness actor route now dispatches once and command-aware transport framing prevents an acknowledgement from being assigned to a different command.
- where: Shipwright/soh/src/zelda3d/behaviors/actor/boss_fd2.{h,cpp}, `boss_fd2_animation_policy.{h,cpp}`; Shipwright/soh/src/overlays/actors/ovl_Boss_Fd2/z_boss_fd2.c; Shipwright/soh/src/zelda3d/model/zelda3d_cmab.{h,cpp}
- gap: Ground-form emergence, idle/turn, fire/claw, burrow, hit→vulnerable, damage, and death CSAB transitions are recovered and ported with independent authored playheads plus OoT3D five-frame crossfades. Every retail persistent action now has one exact initial-CSAB policy; an unknown action invalidates the controller and retains the native draw instead of inventing `vba_wait`. The body is demonstrably submitted and visible, but a fresh matched oracle/SoH image pair is still required before visual parity can close. Capture `_i` dismissed both warnings and drove the exact paired handoff, but falsified the assumption that oracle child `+0x328` is a directly usable rendered-head world anchor: the forced camera showed Link on the oracle while the shipping frame placed a small BossFd2 at the far left against incomplete scene geometry. Recover the oracle post-limb/rendered head transform and a corresponding shipping anchor before the next comparison; do not tune from the actor-local field. Flying Boss_Fd is tracked separately by render.boss-fd-flying.
