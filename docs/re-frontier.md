...
- gap: RE COMPLETE FOR THE SHEATH PATH — ready to port (oot3d-decomp commit 98b3824). All fields named off N64's SHEATH limb case in Player_OverrideLimbDrawGameplayDefault (z_player_lib.c:1425-1446): player[0x1c4] = this->sheathDLists, player[0x1a6] = this->currentShield (row selector, stride 0x10 = 4 entries, matching sSheathWithSwordDLs[shield*4]), gSaveContext[0x80] = equips.buttonItems[0] (0x3B ITEM_SWORD_KOKIRI, 0x3D ITEM_SWORD_BGS, 0x59 ITEM_FISHING_POLE), cfg[0x38] = sDListsLodOffset = lod*2. The +0x40 bump is N64's &sSheathWithSwordDLs[PLAYER_SHIELD_MAX*4] — PLAYER_SHIELD_MAX is 4 so that subscript is entry 16 = offset 0x40 — the child-without-Kokiri-sword fallback row, and the 3DS test cVar2 != 0x3B is N64 line 1437 verbatim. PORT SHAPE: base = sheathDLists + currentShield*4; if (child && sheathType in {16,17} && buttonItems[0] != ITEM_SWORD_KOKIRI) base = fallbackRow; mesh = base[lod*2]; if (mesh != -1) ShowMesh(this, mesh). The -1 rows are the crux of #201 e — they are how OoT3D expresses 'no sword on the back' and the hand-curated Zelda3D_LinkComputeMidMask cannot express it. NEXT (implementation, not RE): write the dedicated zelda3d module implementing reset-then-enable (claim C010) with this sheath rule, replace Zelda3D_LinkComputeMidMask, build, and verify on the FULL user path — a Link with no sword must NOT show one on his back — then post evidence to kanban #201 e before moving it to needs-confirmation. Left unnamed because the sheath port does not need them: the 0x4000 and 0x80000 bits of player[0x29b8].
- notes: 
