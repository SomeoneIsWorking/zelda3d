...
- evidence: NEW 2026-07-28 — the prescribed per-fragment probe was RUN on d9 and it lands. Oracle side (SOH3D_HARNESS_SW=1, SOH3D_PIXEL_TEX=18478200 which is d9's tex0, loadstate scratch/drawiso/zora_masks/probe.state, draw_log to scratch/logs/zora_d9_px.log — 55433 PIXEL lines, nearest fragment per pixel, 7610 px inside d9's mask): texcol=(59.7,65.4,50.0,255) primary=(0.2,69.7,84.5,255) combined=(0.0,51.9,54.6,255). Note PRIMARY's RED IS ZERO — the oracle's vertex colour for this water layer carries no red at all, and the draw therefore adds nothing to red. Prior evidence stands: full-mask ratio ours/oracle over d9 reproduced this session at 0.94/0.74/0.62 (recorded 0.944/0.756/0.644).
- where: unknown — `Shipwright/libultraship/src/fast/zelda3d_sdl3gpu.cpp` (`tevRun`, blend state) / `Shipwright/cmb3d/asset/cmb.cpp`
- gap: DECISIVE NARROWING: d9 and d11 have BYTE-IDENTICAL vertex-lighting and material uniforms — both matDif=(0,0,0,1) matAmb=(1,1,1,0) dir0=(-1.06e-07,-0.9848,-0.17371,1) dif0=(0,0,0,1) amb0=(0.42745,0.42745,0.48627,1), same fog=5/0(104,135,181), same lutS, same tev0=srce300e30/mod000000/op1-1/sc2x1 — and d11 measures 0.977 (at parity) while d9 measures 0.64. So the lighting state, the material state, the fog and the combiner are all RULED OUT as the cause; what differs between the two draws is only tex0 (18478200 64x64 f12 vs 1848a000 256x256 f12) and the vertex colours. Look there. BLOCKER ON THE OBVIOUS NEXT STEP: d9 has ZERO exclusive pixels (every pixel of its mask is also covered by another draw), so tev_mask_ratio --exclusive cannot attribute anything to it, and our FRAGDBG 1/5 frames are whole-frame composites that cannot be compared like-for-like against the oracle's d9-only per-fragment values. For the record the contaminated numbers over the probe's 7610 px were ours tex (111.5,133.0,127.6) / PRIMARY (90.9,147.9,154.6) / final (110.1,148.7,157.8) vs oracle final (103.7,185.0,230.0) — SUGGESTIVE that our PRIMARY carries red where the oracle's has none, but NOT proof, because other draws contribute to those pixels on our side. THE REAL NEXT STEP is a per-fragment dump on OUR side (a Zelda3D_SG equivalent of the sw_rasterizer PIXEL line, emitting texcol/primary/combined per fragment for one selected draw) so both sides are measured as the same quantity. Building that instrument is the task, not another masked-frame comparison.
