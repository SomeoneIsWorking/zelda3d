...
- notes: SWEEP FINDING 2026-07-29 (debug_journal/2026-07-29-adult-gauntlet-plates-never-drawn.md): adult Link's GAUNTLET PLATES are never drawn — mesh ids 4/5/6/17/18/19 appear nowhere in linkAdultMidMask, and there is no strength-upgrade input at all. Same class as #201 e (a state OoT3D consults that our mask does not model) but opposite polarity: #201 e drew something that should be hidden, this hides something that should be drawn. Rule already RE'd: if adult && CUR_UPG_VALUE(UPG_STRENGTH) >= 2 then show 4 and 17, plus 5 or 6 by sLeftHandType == LH_OPEN and 18 or 19 by sRightHandType == RH_OPEN. Confirmed twice: the 3DS test cfg[0x40] == 8 is sRightHandType == PLAYER_MODELTYPE_RH_OPEN because that enum value IS 8; and OoT3D's always-on core set {45,46,47} matches our mask's independent LINK_MID(45)|LINK_MID(46) start, because both index the same link_v2.cmb by mesh position. Check whether 47 is our far-LOD mesh before adding it. NOT YET FIXED because verification needs UPG_STRENGTH forced the way bitem forces the B item and no such primitive exists — shipping it unverified would repeat #201 e's first failed measurement.
