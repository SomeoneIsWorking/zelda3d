...
- gap: SHEATH SELECTOR IDENTIFIED (oot3d-decomp commit 142d37b). Helper 0x004c70c4 decides what is worn on Link's BACK — player[0x1b6] is sSheathType and its branch values 0x10..0x13 match N64's PLAYER_MODELTYPE_SHEATH_16..19 verbatim (z64player.h:310-313), with 0x12/0x13 being the shield-on-back pair. That makes it exactly the #201 e mechanism. Also named, matching N64's Player_SetModelGroup quartet: player+0x1b4 sLeftHandType, +0x1b5 sRightHandType, +0x1b6 sSheathType, +0x1b7 sWaistType. And the table index is settled: the final lookup is table[cfg[0x38]] with cfg = 0x0053c924, and Player_DrawImpl sets cfg[0x38] = param_2 << 1 = N64's sDListsLodOffset = lod * 2 — so rows are indexed by LOD, are 0x10 bytes apart, and are selected by player[0x1a6]; AGE enters through which row base is chosen (0x0053c4b8 / 0x0053c4d8 / 0x0053c5e8 / player[0x1c4] + player[0x1a6]*0x10, with +0x40 and +0x80 variants), not through the index. A previous revision of the doc read these tables as adult/child pairs; that is corrected in place. REMAINING before the port: name player[0x1c4] (per-instance table base), player[0x1a6] (row selector), gSaveContext[0x80] (compared against 0x3B, 0x59, 0x3D — likely a scene or cutscene id) and the 0x4000 / 0x80000 bits of player[0x29b8]; then port into a dedicated zelda3d module replacing Zelda3D_LinkComputeMidMask and verify on the full user path — a Link with no sword must not show one on his back.
- notes: 
