...
- notes: **RE-SCOPE PRECONDITION CHECKED 2026-07-28: this step is READY TO RUN, no build needed.** The four unresolved deps (`player[0x2c]`/`[0x10c]` semantics, the `*(0x0053a07c+0x28)` runtime-mode gate, `FUN_00359690`, `player[0x81]`) still need live observation of oracle Player memory while Link walks up the Kakariko stairs — but everything required for that is already in place: `scratch/gameplay_settled.state` is cached (so `boot_to_gameplay`'s fast path works) AND the harness binary is built at `Azahar/build-harness/bin/Release/soh3d_harness` (157 MB, 2026-07-22); `ninja -C Azahar/build-harness soh3d_harness` is a no-op. Verified running: `harness_ctl.py send gameplay` answers.
  **ONE GOTCHA that costs a confusing failure:** `harness_ctl.py` needs the repo `.env` sourced first (`set -a; . ./.env; set +a`) or the harness exits immediately and the driver reports the useless `RuntimeError: harness closed stdout unexpectedly`. Set `HARNESS_STDERR=<path>` to see the real reason.
  **CORRECTION — an earlier revision of this note claimed the binary was NOT built and that the step was therefore session-opening. Both halves were wrong.** The "not built" came from `find Azahar -maxdepth 3 -name soh3d_harness`, which cannot reach a binary at depth 4 (`build-harness/bin/Release/...`); the follow-up "it does not run" came from a spawn failure that was really the missing `.env`. Two shallow checks, two wrong conclusions, one wrong note committed. Verify a binary's absence by looking at the path the build script actually writes (`harness_bin` in `tools/soh3d_harness.sh`), not by a depth-limited find.
