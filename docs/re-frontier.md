...
- notes: jump ruled out (2026-07-16 journal §3). ROOT-MOTION CHECK DONE (journal §5c, csab_rootmotion probe over all 15 zelda_horse CSABs): the TITLE GALLOP `hl_anim_fastrun2_30` is IN-PLACE per loop (netZ=−5.6; the 797 range is gait oscillation) → gallop is speed-driven, NOT the cause. BUT `hl_anim_slowrun2_30` (netZ=−50) and the transition clips (−61/−88) DO carry net root motion. So cause (a) holds IFF the 0x41 cue plays slowrun/a-transition (not gallop). Decisive remaining step: resolve WHICH clip FUN_002535f0's +0x1b0 index selects during 1380–1619 (Ghidra: index into the iRam00253750 table) — gallop→cause (b) cue-decode, slowrun/transition→cause (a) apply clip root motion.
