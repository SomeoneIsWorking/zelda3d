...
- gap: The exact disabled zero/zero branch is ported and close-tested for the five retail materials that consume a fragment source with lighting disabled. The 197 enabled primary and 69 enabled secondary consumers remain RE-partial: first associate a visible, enabled CMB material with its shipping draw class, then cache that draw's complete PICA config/light/LUT probe before implementing an enabled fixed-function calculation. The descriptor-colliding Kokiri `zelda_wood02.zar` test is not that association; it needs a positive identity control. The candidate library `CmbRenderer`, recovered `FUN_004c7ab0` model-submission helper, direct `FUN_003fbba8` material-state route, and generic `FUN_003fcc70` bridge are ruled out for the current fixture.
