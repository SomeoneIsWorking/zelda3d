...
- gap: #206 restored the MECHANISM (feetPos is fed again so ActorShadow_DrawFeet has somewhere to draw) but OoT3D's shadow is roughly 4x the contrast and visibly larger and softer. Do NOT tune shadowAlpha/shadowScale to close this — RE what OoT3D actually draws first ([[soh3d-re-and-port-not-fix-diffs]]). Open questions: (a) does Grezzo still use the N64 ActorShadow_DrawFeet/DrawCircle pair, or a different projected-shadow draw? oot3d-decomp has NO ported shadow function yet — only divergence_map.md:389 'Player draw-shadow / motion-blur trail recorder' at 0x0036c494/0x0036f4f0, which is the TRAIL recorder, not the shadow; (b) is the difference alpha, scale, the shadow texture, or a soft/blurred 3DS-only draw? CAVEAT: the oracle and Zelda3D cameras/poses are not identical, so the sample boxes do not cover identical geometry — the before/after/oracle ORDERING is solid, the exact ratio is not. Next step: decompile the OoT3D shadow draw starting from the ActorShape shadowDraw fn-ptr in docs/actor_layout.md.
- notes: 
