...
- gap: **the port is not landed, BUT the 418-line FUN_00239fd8 body port is NOT what's needed** (major scoping correction 2026-07-17). The ~28-unit Δeye-Y divergence is SOLELY in **Camera_CalcAtDefault** (3DS FUN_00338ac8 vs SoH z_camera.c:906): the 3DS adds an extra at-Y term `atTarget.y += player[0x1760] · −0.01f` gated on `player[0x29b8] & 0x100`; SoH's has NO such term (z_camera.c:929). Independently re-derived via decomp-diff 2026-07-17 AND already documented in `oot3d-decomp/docs/gameplay_firstdiv.md:1120-1179`. Everything else matches: yOffset formula identical, both pitch clamps identical (0x38A4/−0x3C8C), LERP rates (0.1/0.2) identical. Port target is a shared `behaviors/camera/at_default.cpp` (Camera_CalcAtDefault is called by Normal0/1/2 + Jump1), NOT normal1.cpp — Normal1Behavior::update can stay a no-op delegate. BLOCKER (narrowed 2026-07-17): `player[0x1760]` is a Grezzo 3DS-only accumulated+decaying camera **Y-bias**. Writer RE'd = `FUN_00250ad0` (Player func): SET (+flag 0x100) when a state (`player[0x1708]==DAT_00251cd8`) + a Y-delta threshold hold, accumulating the delta; DECAYS each frame (`−=[DAT_0025293c+0x90]`) until it clears the flag — a camera Y-smoothing that lags Link's elevation changes (stairs/slopes ⇒ ~2500→−25 at Kakariko). Faithful port = reimplement the accumulate/decay in the SoH Player + apply `at.y += bias·−0.01` in the at_default seam. NEXT: resolve the writer constants + map player[0x2c]/[0x10c]/[0x1708] to SoH Player fields (journal `2026-07-17-oot3d-dat-constants...`, `gameplay_firstdiv.md:1120-1179`). Do NOT approximate with a magic −25.
