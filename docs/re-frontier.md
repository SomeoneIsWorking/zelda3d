...
- gap: RE COMPLETE — IMPLEMENTATION ONLY REMAINS (oot3d-decomp 7175128, claim C011). The missing rule: child && sheathType in {SHEATH_16, SHEATH_17} && buttonItems[0] != ITEM_SWORD_KOKIRI -> draw NOTHING on the back, or the Deku shield alone if that shield is equipped. Positively identified, not inferred: the 3DS fallback table 0x0053c4b8 is byte-for-byte N64's two '(child, no sword)' rows in sSheathWithSwordDLs (z_player_lib.c:212-223), NULL-vs-value matching at every position with -1 as the no-op sentinel. WHY OUR CODE CANNOT DO IT TODAY: LinkMidMask::compute maps every sheathType to some mesh, and the LinkGear POD does not carry equips.buttonItems[0] at all — so this is a MISSING STATE, not a wrong mesh id, and re-curating the mesh-id map could never fix it. TO DO: thread the B-button item (or a hasKokiriSword flag) into LinkGear so both the OoT adapter and the future MM one can use it; prefer porting the TABLE shape (rows of four indexed lod*2, -1 sentinel, fallback-row swap) over extending the enum chain, since the table is what OoT3D runs and generalises to the other rows free; the rule is child-only, adult behaviour unchanged. VERIFY ON THE FULL USER PATH, not a forced state: a fresh child Link who has not picked up the Kokiri sword shows nothing on his back, and shows it again after collecting it. Then post evidence to kanban #201 e and move it to needs-confirmation.
- notes: 
