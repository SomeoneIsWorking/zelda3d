...
- evidence: DRAW ISOLATION BUILT AND USED 2026-07-28 (instrument I003). Our draw groups match the oracle's PICA draws 1:1 by VERTEX COUNT in append order (claim C005): oracle d9 nv=1107 IS our group index 8, confirmed by isolation covering 98.07% of d9's mask. With sgdrawonly 8 + FRAGDBG 1/5/6, our side finally measures the same quantity as the oracle's per-fragment probe. Numbers over the 7610 px where the oracle reports d9 as nearest fragment, converted to the oracle's linear 8-bit convention (our FRAGDBG readback is GAMMA-ENCODED — instrument I004, do not compare raw): texcol ours (40,60,55) vs oracle (59.7,65.4,50.0); primary ours (26,74,82) vs oracle (0.2,69.7,84.5); combined ours (26,66,67) vs oracle (0.0,51.9,54.6).
- where: unknown — `Shipwright/libultraship/src/fast/zelda3d_sdl3gpu.cpp` (`tevRun`, blend state) / `Shipwright/cmb3d/asset/cmb.cpp`
- gap: NARROWED TO THE RED CHANNEL (claim C006). G and B agree within a few units in texcol, primary AND combined — so whatever is wrong is not a broad gain or a blend/alpha error. Two red-channel divergences remain: (1) our PRIMARY carries red 26 where the oracle's is 0.2, i.e. the oracle's vertex colour for this water layer is red-free and ours is not; (2) our texcol red is ~33% short (40 vs 59.7). Since the draw is ADDITIVE, an unwanted red in PRIMARY adds red the oracle never adds. NEXT: chase the vertex-colour red first — it is the larger and cleaner signal. Read the water material's vertex colours out of the CMB at the data level (Shipwright/cmb3d) rather than through the renderer, and compare against what the oracle's vertex-lighting produces: with matDif=(0,0,0,1), dif0=0 and matAmb=(1,1,1,0) x amb0=(0.42745,0.42745,0.48627), the oracle's PRIMARY ~ vtxColour x ambient, so a red-free PRIMARY implies a red-free vertex colour. Check our CMB vertex-colour decode for that material (component order, scale, or a default-white fallback when a colour attribute is absent). CAVEAT to carry: the colour-space conversion above is an inverse-sRGB applied on our side and has NOT been validated against a known ramp — if it is wrong, which channels look divergent could change. Validating it is cheap and should be step one.
