...
- gap: BASE RULE TABLE DECODED (oot3d-decomp commit fd9cbfc). 0x004dc388 is a single 32-byte record of four adult/child INTERLEAVED pairs — adult {45,45,46,47}, child {24,24,25,26} — and the base loop in 0x004c4560 tests each mesh index against the four entries for the current linkAge, setting the flag to 1 on a match and 0 otherwise. CORRECTION carried in the doc: an earlier note called these four 'is-zero' tests, which came from Ghidra rendering cmp r1,r4 as uVar8==uVar9 with the loop counter starting at zero; the ARM compares against the MESH INDEX, so the table holds mesh indices. BLOCKING QUESTION before any port — the flag's POLARITY is NOT established. SetMeshVisible was named from context; the code only does flags[i] = (v != 0). If 1 means visible, the base loop leaves three meshes drawn; if 1 means hidden, it culls the three age-specific variants and shows the rest. Those give OPPOSITE ports. Settle it from the consumer — the array is at *(*(obj+0x27c)+0x14)+0x6c with its count at +0x68, so find the reader in the mesh-draw path — or read the array live from the oracle for a Link whose visible parts are known. Then: decode the id tables at 0x0053c5a8 / 0x0053c6ec and player[0x1bc]; RE helpers 0x004c71dc and 0x004c70c4; identify selector fields 0x1b4/0x1b5 (hand types), 0x1ac, 0x1a6, 0x1749, 0x121c, 0x1710, 0x6c, 0x29b8; port into a dedicated zelda3d module and verify on the full user path.
- notes: 
