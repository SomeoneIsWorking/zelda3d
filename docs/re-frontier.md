...
- notes: **RE-SCOPE PRECONDITION CHECKED 2026-07-28, and it is a SESSION-OPENING task, not a tail-of-session one.** The re-scope the 2026-07-22 note asks for has no cheap version: every one of the four unresolved deps (`player[0x2c]`/`[0x10c]` semantics, the `*(0x0053a07c+0x28)` runtime-mode gate, `FUN_00359690`, `player[0x81]`) can only be disambiguated by reading oracle Player memory live while Link walks up the Kakariko stairs. `scratch/gameplay_settled.state` IS cached (so `boot_to_gameplay`'s fast path is available), but the harness BINARY is not built — `Azahar/build-harness/soh3d_harness` is absent, and building the embedded Azahar core is a heavy operation. So: start this step at the TOP of a session with `tools/soh3d_harness.sh`, not at the end of an unrelated arc. Nothing about the 2026-07-22 unblocking claim is falsified — it just is not free.
