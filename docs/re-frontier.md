...
- status: in-progress
- deps:
- evidence: oot3d-decomp/docs/boss_fd2.md and decomp functions 003B4308/00209588/00316DC0/0036932C/003C724C/00212F94/0024E4E8/0024E700/0036442C/0036FCA8; live fdinfo/fddeath/vbball/vbinfo positive and negative discriminators; exact update discriminator `bodyLead 118 -> 88` after 120 updates; scratch/screenshots/boss_fd_flight_materials_clear.png, boss_fd_death_head_materials.png, boss_fd_attack_shadow_unified_tev_fixed.png, boss_fd_detached_bone_settled_final.png, boss_fd_rib_impact_3ds.png, volvagia_oracle_cutscene360.png, and volvagia_native_updatecadence_live.png; unified shader all-variant selftest, harness boot/step gate, and zelda3d_app build pass
- where: Shipwright/soh/src/zelda3d/behaviors/actor/boss_fd.{h,cpp}; Shipwright/soh/src/zelda3d/behaviors/actor/en_vb_ball.{h,cpp}; Shipwright/soh/src/zelda3d/anim/zelda3d_anim.cpp
- gap: Ordinary flight has independent authored body/head/arm CSAB playheads, recovered 150-entry body and 45-entry mane histories, eighteen procedural body bones, separate head/arms, three mane chains, and all seven flying-part CMAB bindings. The 3DS-owned histories/controllers now advance from the actor post-update seam, matching recovered `FUN_003C724C` ownership and remaining independent of render visibility/cadence. The death transition masks the exact trailing body mesh IDs, replaces them with eighteen rigid death-body CMB instances using the 3DS distance-derived matrices, and selects the death-head CMB at action 204. The recovered `FUN_0014690C` five-pass effect draw submits `vb_particle_group.cmb` for fire/skull/debris/smoke/ember with exact mesh IDs, material constant slot 4, billboard-vs-rigid transforms, CMABs, and a 110-instance cap. En_Vb_Ball draws the recovered attack-stone, detached-death-body, and keep-shadow models and ports the 3DS collision producers. Typed base gameplay records supply only gameplay values; no N64 display list, animation state, phase, joints, morph, or history enters a 3DS object. Natural oracle/native captures now agree on the long articulated title-card body; precise paired image comparison remains open.
- notes: Typed fdfly enters the real action and resets only the 3DS-owned histories; fdinfo scans 150 3DS body samples, reports the 180-record effect denominator plus debris/smoke counts, and refuses non-Boss_Fd selections. fddeath validates live/death segment counts and the action-204 head switch while refusing invalid actors/ranges. `fdfx <3dsType 0..5> <count 1..12>` holds controlled particle records and reports the inspected/staged denominator; invalid type 6 produced `staged 0/110`. `vbball` refuses non-Boss_Fd selections/invalid params and holds one typed En_Vb_Ball branch; `vbinfo` reports typed shadow/rotation/collision state. Live impact discriminators produced large `debris=6 smoke=4`, ordinary `debris=2 smoke=0`, rib `debris=0 smoke=4`, shadow `255`, and rib rotVel `(-14.51,-22.65)`; disabling Zelda3D on the identical ordinary-impact path produced the N64 control `debris=5`. The natural intro A/B caught draw-owned history: `step 60` formerly advanced one sample and visibly compressed the body; the post-update seam advances one sample per gameplay update, with a 120-step modulo discriminator and a live long-body capture. The initial radial "geometry corruption" diagnosis was false: params-101 captures still contained older params-100 actors, and first-frame teleports retained an interpolated transform. The persistent discriminator was unified-on versus legacy; it isolated the real missing generic-TEV material path.
