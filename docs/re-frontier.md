...
- gap: THE REAL MECHANISM IS LOCATED: 0x004c4560, the equipment-visibility driver (oot3d-decomp/docs/player_draw_impl_located.md, commit 1a456b0). It is called first thing in Player_DrawImpl and it walks EVERY mesh, setting visible/hidden from a rule table at 0x004dc388, then applies per-state overrides from tables at 0x0053c5a8 and 0x0053c6ec. So OoT3D drives mesh visibility from DATA every frame — exactly what the hand-curated Zelda3D_LinkComputeMidMask stands in for. Two dead ends worth not repeating: (1) the Player_DrawImpl block previously RE'd is gauntlets/boots/bracelet, NOT the sword; (2) Player_OverrideLimbDrawGameplayDefault = 0x004c2b04 is a FOUR-INSTRUCTION STUB that returns 0 — Grezzo did not port N64's per-limb display-list swapping, so there is nothing there to find. REMAINING before any port: decode the per-mesh rule table at 0x004dc388 (eight u32s per entry, four is-zero tests, selector from gSaveContext+4 = linkAge) — that table is the heart of #201 e; decode the id tables at 0x0053c5a8 / 0x0053c6ec and the per-instance table at player[0x1bc]; RE the helpers 0x004c71dc and 0x004c70c4; identify the Player selector fields 0x1b4/0x1b5 (hand types), 0x1ac, 0x1a6, 0x1749, 0x121c, 0x1710, 0x6c, 0x1bc, 0x29b8. Then port into a dedicated zelda3d module and verify on the full user path.
- notes: 
