...
- status: todo
- deps: 
- evidence: Kokiri Forest entrance 0xEE, daytime 0x6000. Shadow contrast = (green under-boots)/(green clean grass): ORACLE OoT3D 58.1/88.8 = 0.65 (35% darkening); ZELDA3D after the #206 fix 87.5/95.7 = 0.91 (9%); ZELDA3D before the fix 102.9/102.3 = 1.01 (no shadow at all). Oracle frame scratch/screenshots/oracle_kday.png, ours scratch/screenshots/shadow206_after.png.
- where: z_player.c Player_Draw (the #206 limb-walk re-run) + z_actor.c ActorShadow_DrawFeet/DrawCircle
- gap: PREMISE FALSIFIED — do NOT port from the first write-up. The OoT3D shadow MATH is N64's, unchanged: 0x0033e450 is ActorShadow_DrawFoot, not a Grezzo rewrite, and SoH already implements its light-yaw rotation, its 4.5 - dir.y*0.035 stretch and its min(1, w*5e-5)*shadowAlpha darkening at z_actor.c:160. See oot3d-decomp/docs/actor_shadow.md (corrected in dd2faa3; the superseded claim is in 7d82eb4). Only two things are genuinely 3DS-specific and neither obviously moves contrast: the draw target (a cached model object, asset 0x51, instead of gFootShadowDL) and a Matrix_Translate(0,1.5,0) z-fight guard. So the measured gap (contrast 0.91 ours vs 0.65 oracle, green under-boots / green clean grass) has an UNKNOWN cause and the formula is not it. Investigate the live INPUTS on both sides before changing anything: (1) lightWeight = (col.r+col.g+col.b)*dir.y per light, saturating at 20000 via min(1, w*5e-5) — fewer/dimmer lights fade the shadow exactly this way; (2) light->l.dir[1], which ALSO drives the stretch — our shadow reads round and small where the oracle's is long, which is the signature of a different dir.y rather than a missing formula; (3) the 0x51 sprite vs gFootShadowDL (size/softness/opacity ramp); (4) Player's installed shape.shadowAlpha / shadowScale. NOTE several lighting rows in docs/parity-map.md are CLOSED and off-limits to tuning — measure first, and if the cause lands inside a closed row, report it rather than retuning.
- notes: 
