...
void Fast::Zelda3DRenderer::releaseGpuResources(bool (*note)(const void* handle, const char* what)) {
    ...
    for (auto& kv : g_models) {
        SgModel& m = kv.second;
        if (m.vbo != nullptr && note(m.vbo, "zelda3d model vbo")) {
            SDL_ReleaseGPUBuffer(g_device, m.vbo);
        }
        m.vbo = nullptr;
        for (SDL_GPUTexture* t : m.textures) {
            ...
            if (t != nullptr && note(t, "zelda3d model texture")) {
    ...
    for (auto& kv : g_pipelines) {
        if (kv.second != nullptr && note(kv.second, "zelda3d pipeline")) {
            SDL_ReleaseGPUGraphicsPipeline(g_device, kv.second);
        }
    }
    g_pipelines.clear();

    for (auto& kv : g_uniPipelines) {
        if (kv.second != nullptr && note(kv.second, "zelda3d unified pipeline")) {
            SDL_ReleaseGPUGraphicsPipeline(g_device, kv.second);
        }
    }
