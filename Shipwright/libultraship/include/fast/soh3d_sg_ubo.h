#ifndef SOH3D_SG_UBO_H
#define SOH3D_SG_UBO_H

// SoH3D SDL3 GPU per-draw uniform layout — the single source of truth shared by the renderer
// (soh3d_sdl3gpu.cpp) and the layout unit tests (tests/soh3d_render_tests.cpp).
//
...
#include <cstddef>
#include <cstdint>
#include "fast/soh3d_gl.h" // SOH3D_GL_MAX_BONES — shared by the shader string, this struct, the tests
...
struct SgUbo {
    ...
    float uFog2[4];
    float uAmbient[4];
    float uBones[SOH3D_GL_MAX_BONES * 16]; // MUST stay last: pushed as its own <=4096 B block
};
...
static_assert(kCommonBytes <= kMaxUboSectionBytes,
              "SoH3D common UBO block exceeds SDL3 GPU's 4096-byte push cap; split it further");
static_assert(kBonesBytes <= kMaxUboSectionBytes,
              "SoH3D bone UBO block exceeds SDL3 GPU's 4096-byte push cap; move bones to a "
...
// gSoH3dFaceCullFlip (default 0). Returns true if the winding should be CLOCKWISE.
