// Regression tests for the SoH3D SDL3 GPU render fixes (terrain/world rendering on the sole backend).
//
...
#include "fast/soh3d_sg_ubo.h"

using namespace SoH3DSg;
...
TEST(SoH3DUboLayout, PushBlocksFitSdl3GpuSectionCap) {
...
TEST(SoH3DUboLayout, BlocksTileTheStructContiguously) {
...
TEST(SoH3DUboLayout, BoneBlockIsExactlyTheSectionCapAt64Bones) {
    EXPECT_EQ(kBonesBytes, (uint32_t)SOH3D_GL_MAX_BONES * 16 * sizeof(float));
    EXPECT_LE((uint32_t)SOH3D_GL_MAX_BONES * 16 * sizeof(float), kMaxUboSectionBytes);
}
...
TEST(SoH3DUboLayout, CommonFieldOffsetsMatchStd140) {
    ...
    EXPECT_EQ(offsetof(SgUbo, uBones), 320u);
}
...
TEST(SoH3DUboLayout, TintSkinIsWithinPushedCommonRange) {
    EXPECT_LE(offsetof(SgUbo, uTintSkin) + sizeof(SgUbo::uTintSkin), kCommonBytes);
}

// BUG 2: with the default face-cull flip (gSoH3dFaceCullFlip == 0), front faces must be CCW, matching
// OoT3D's winding. CW-front (the old default) back-culls terrain and the sky dome.
TEST(SoH3DWinding, DefaultIsCounterClockwiseFront) {
