...
// So PlayerBehavior is reached through that hook: the extern "C" shims at the bottom of soh3d_link.cpp
...
// The low-level CMB skeleton FK retarget primitives (SoH3D_UpdateAnimN64*, SoH3D_PosedGroundOffset,
// ...) live in soh3d_model.cpp; these subsystems CALL them.
#ifndef SOH3D_BEHAVIORS_ACTOR_PLAYER_H
#define SOH3D_BEHAVIORS_ACTOR_PLAYER_H

#include "../actor_behavior.h"
#include "../../soh3d_link.h" // SoH3dBoneCorr (C-ABI retarget correction record)
#include <cstdio>             // FILE*
...
// retarget-correction derivation (tools/soh3d_link_retarget_derive.py).
class LinkJointDump {
public:
    FILE* file = nullptr;
    int remaining = 0;
...
// fresh A edges until heldActor is set. Run from SoH3D_WalkInject just before Play_Update.
...
public:
    ...
    // Link-specific per-frame hooks + REPL, driven from soh3d.c via the extern "C" shims.
    ...
    int modelId = -1; // last modelId the player body drew with (REPL pose scanner / SoH3D_LinkModelId)
