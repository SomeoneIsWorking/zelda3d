// SoH3D behavior: Obj_Comb (field beehive) — model REPLACEMENT.
//
...
#define SOH3D_COMB_ZAR "/actor/zelda_field_keep.zar"
#define SOH3D_COMB_CMB "Model/hatisu_model.cmb"
...
bool ObjCombBehavior::tryDrawModel(PlayState* play, Actor* actor) {
    ...
    if (sModelId == 0) {
        sModelId = SoH3D_AutoModelId(SOH3D_COMB_ZAR "|" SOH3D_COMB_CMB);
    }
    if (sModelId < 0) {
        return false; // no OoT3D beehive CMB -> let the N64 beehive draw
    }
    SoH3D_DrawActorModel(play, sModelId, actor, SoH3D_GScale(kCombGScaleSlot, kCombWorldScale));
    return true;
}
