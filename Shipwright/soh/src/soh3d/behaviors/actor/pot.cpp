// SoH3D behavior: En_Tubo_Trap (flying-trap pot) — model REPLACEMENT.
//
...
#define SOH3D_POT_ZAR "/actor/zelda_dangeon_keep.zar"
#define SOH3D_POT_CMB "Model/tubo_model.cmb"
...
bool EnTuboTrapBehavior::tryDrawModel(PlayState* play, Actor* actor) {
    ...
    if (sModelId == 0) {
        sModelId = SoH3D_AutoModelId(SOH3D_POT_ZAR "|" SOH3D_POT_CMB);
    }
    if (sModelId < 0) {
        return false; // no OoT3D pot CMB -> let the N64 pot draw
    }
    SoH3D_DrawActorModel(play, sModelId, actor, SoH3D_GScale(kPotGScaleSlot, kPotWorldScale));
    return true;
}
