// SoH3D behavior: Bg_Bombwall (bombable cracked wall) — model REPLACEMENT.
//
...
#define SOH3D_BOMBWALL_ZAR "/actor/zelda_field_keep.zar"
#define SOH3D_BOMBWALL_CMB_INTACT "Model/c_bombwallbefore_model.cmb"
#define SOH3D_BOMBWALL_CMB_BROKEN "Model/c_bombwallafter_model.cmb"
...
bool BgBombwallBehavior::tryDrawModel(PlayState* play, Actor* actor) {
    ...
    if (sIntactId == 0) {
        sIntactId = SoH3D_AutoModelId(SOH3D_BOMBWALL_ZAR "|" SOH3D_BOMBWALL_CMB_INTACT);
    }
    if (sBrokenId == 0) {
        sBrokenId = SoH3D_AutoModelId(SOH3D_BOMBWALL_ZAR "|" SOH3D_BOMBWALL_CMB_BROKEN);
    }
    ...
    SoH3D_DrawActorModel(play, modelId, actor, SoH3D_GScale(kBombwallGScaleSlot, kBombwallWorldScale));
    return true;
}
