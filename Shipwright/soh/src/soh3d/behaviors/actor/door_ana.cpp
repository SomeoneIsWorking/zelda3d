// SoH3D behavior: Door_Ana (grotto hole / trapdoor entrance) — model REPLACEMENT.
//
...
#define SOH3D_DOOR_ANA_ZAR "/actor/zelda_field_keep.zar"
#define SOH3D_DOOR_ANA_CMB "Model/ana01_modelT.cmb"
...
bool DoorAnaBehavior::tryDrawModel(PlayState* play, Actor* actor) {
    ...
    if (sModelId == 0) {
        sModelId = SoH3D_AutoModelId(SOH3D_DOOR_ANA_ZAR "|" SOH3D_DOOR_ANA_CMB);
    }
    if (sModelId < 0) {
        return false; // no OoT3D grotto-hole CMB -> let the N64 hole draw
    }
    SoH3D_DrawActorModel(play, sModelId, actor, SoH3D_GScale(kDoorAnaGScaleSlot, kDoorAnaWorldScale));
    return true;
}
