// SoH3D behavior: Obj_Hamishi (silver rock — the Megaton-Hammer-smashable boulder) — model
// REPLACEMENT.
//
...
#define SOH3D_HAMISHI_ZAR "/actor/zelda_field_keep.zar"
#define SOH3D_HAMISHI_CMB "Model/obj_isi01_model.cmb"
...
bool ObjHamishiBehavior::tryDrawModel(PlayState* play, Actor* actor) {
    ...
    if (sModelId == 0) {
        sModelId = SoH3D_AutoModelId(SOH3D_HAMISHI_ZAR "|" SOH3D_HAMISHI_CMB);
    }
    if (sModelId < 0) {
        return false; // no OoT3D rock CMB -> let the N64 rock draw
    }
    SoH3D_DrawActorModel(play, sModelId, actor, SoH3D_GScale(kHamishiGScaleSlot, kHamishiWorldScale));
    return true;
}
