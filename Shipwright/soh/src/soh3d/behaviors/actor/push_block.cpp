// SoH3D behavior: Obj_Oshihiki (push/pull block) — model REPLACEMENT.
//
...
#define SOH3D_PUSHBLOCK_ZAR "/actor/zelda_dangeon_keep.zar"
...
int resolveBrick(int brick) {
    ...
    if (sIds[brick] == 0) {
        char path[160];
        snprintf(path, sizeof(path), "%s|%s", SOH3D_PUSHBLOCK_ZAR, kBrickCmb[brick]);
        sIds[brick] = SoH3D_AutoModelId(path);
    }
    return sIds[brick];
}
...
bool ObjOshihikiBehavior::tryDrawModel(PlayState* play, Actor* actor) {
    ...
    float scale = sizeScale(sz) * SoH3D_GScale(kPushBlockGScaleSlot, 1.0f);
    SoH3D_DrawActorModel(play, modelId, actor, scale);
    return true;
}
