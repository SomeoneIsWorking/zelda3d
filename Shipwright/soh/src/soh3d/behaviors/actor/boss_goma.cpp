...
// produces them (NOT by forcing shape.rot directly). See SoH3D_BossGomaForceClimb below.
extern "C" {
...
// in soh3d.c). BossGoma_WallClimb itself drives shape.rot.x -> -0x4000, world.rot.y -> wallYaw+0x8000
...
// climb has room to run); hold!=0 keeps her there indefinitely (see SoH3D_BossGomaClimbTick). Returns
// 1 if applied (selection was a Boss_Goma), else 0.
extern "C" int SoH3D_BossGomaForceClimb(Actor* actor, float climbY, int hold) {
    if (actor == NULL) {
...
// Per-frame hold (called from SoH3D_ActorPostUpdate for every actor; self-gates on id + hold flag).
...
extern "C" void SoH3D_BossGomaClimbTick(Actor* actor) {
...
extern "C" int SoH3D_BossGomaClimbHeld(void) {
