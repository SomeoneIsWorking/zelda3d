...
#include "soh3d_walk_stop_sweet.inc"
...
void SoH3D_UpdateAnimAuto(int modelId, const char* animName, float rate, float n64CurFrame,
    ...
    if (incomingWalkEnd && csabChanged &&
        ...
        // soh3d_walk_stop_sweet.inc), pick the nearer end, and size the cross-fade so each morph frame
        ...
        // CSAB<->leg-phase offset K=0 which does NOT hold for SoH3D's single-CSAB (non-blend) walk, so
        ...
        // is decoupled from SoH3D's free-run φ). Preserve the incoming name's prefix/suffix (cl_ / _free)
