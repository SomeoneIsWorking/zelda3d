...
#include "soh3d_model_internal.h" // LoadedModel + loadModel (shared with soh3d_anim.cpp)
...
#include <stb_image.h>
#include "soh3d_stairs.h" // procedural stair geometry (gSoH3dStairs, generateStairsGroup, ...)
...
// LoadedModel moved to soh3d_model_internal.h (shared with soh3d_anim.cpp).
...
static inline SoH3DGlGroup makeCgroup(const SoH3D::Cmb& cmb, const SoH3D::CmbDrawGroup& g,
                                      const SoH3D::CmbVertex* srcVerts, int texBase) {
    return SoH3D::MakeGlGroup(cmb, g, srcVerts, texBase);
}
...
static int appendTextures(LoadedModel* out, const SoH3D::Cmb& cmb, std::vector<std::pair<int,int>>* dims = nullptr) {
    if (dims) return SoH3D::AppendCmbTextures(cmb, out->texRgba, *dims);
    std::vector<std::pair<int,int>> scratch;
    return SoH3D::AppendCmbTextures(cmb, out->texRgba, scratch);
}
...
} // end anonymous namespace — loadModel needs EXTERNAL linkage (declared in soh3d_model_internal.h
  // so soh3d_anim.cpp can resolve a model). It still sees the internal-linkage loaders above
