...
// `boots [n]` — read or set the equipped boots. Boots add their own meshes, so like `bitem`
        ...
        if (n < 2) {
            ...
            if (!Zelda3D_Sg_GroupBounds(mid, gidx, mn, mx)) {
                Zelda3D_ReplReply(outPath, "camdraw: model %d group %d has no bounds (not uploaded, "
                                           "or the group has no vertices)", mid, gidx);
            } else {
                ...
                if (dist <= 0.0f) dist = (ext > 1.0f ? ext : 1.0f) * 1.8f;
