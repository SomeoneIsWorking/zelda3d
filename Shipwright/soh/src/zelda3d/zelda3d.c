...
static Actor* gZelda3dPendingActor = NULL;
static int gZelda3dPendingModel = -1;
static float gZelda3dPendingScale = 1.0f;
static float gZelda3dPendingGroundOff = 0.0f;
static int gZelda3dPendingAuto = 0; // 1 = auto-replaced (apply the rig-mismatch guard); 0 = hand-verified table entry
...
#include "zelda3d_scene_names.inc"
...
static const Zelda3DBoneMap* gZelda3dPendingBoneMap = NULL;
...
static const char* gZelda3dPendingAnimOtr = NULL;
...
static float gZelda3dPendingN64CurFrame = 0.0f;
static float gZelda3dPendingN64AnimLength = 0.0f;
...
static float gZelda3dPendingMorphWeight = 0.0f;
...
static Vec3f sZelda3dActorPinPos;
static Vec3s sZelda3dActorPinRot;
...
static s32 sZelda3dSelDrawModel = -1;
static float sZelda3dSelDrawScale = 1.0f;
static float sZelda3dSelDrawGroundOff = 0.0f;
...
static s32 sZelda3dSelDrawDsHave = 0;
static float sZelda3dSelDrawDsLiftY = 0.0f;
static float sZelda3dSelDrawDsLocal[3] = { 0.0f, 0.0f, 0.0f };
...
static FILE* sZelda3dMotionFile = NULL;
static Actor* sZelda3dMotionActor = NULL; // pinned at asample time so reselecting doesn't hijack it
static s32 sZelda3dMotionRemaining = 0;
static s32 sZelda3dMotionFrame = 0;
...
// in libultraship/src/fast/zelda3d_hud_vk.cpp) owns the Vulkan textured-quad drawing.
extern int  Zelda3D_Hud_Available(void);
extern int  Zelda3D_Hud_Begin(int* outW, int* outH);
extern int  Zelda3D_Hud_Tex(const void* key, const void* rgba, int w, int h);
extern void Zelda3D_Hud_Draw(int tex, float x, float y, float w, float h, float u0, float v0, float u1,
                           float v1, unsigned int tintRGBA);
extern void Zelda3D_Hud_End(void);
...
static void Zelda3D_InitForceTime(void) {
    static int done = 0;
    const char* v;
    if (done) {
        return;
...
static int Zelda3D_TerrainWarpEnabled(void) {
    ...
    return cached && gZelda3dTerrainWarp && !Zelda3D_CollisionEnabled();
}
...
static float Zelda3D_N64FloorCb(float x, float z) {
    Vec3f pos;
...
static float Zelda3D_ReconcileCutsceneCam(PlayState* play) {
    const char* sceneName;
    int modelId;
    float meshY, deficit;
...
static void Zelda3D_EmitModelDraw(PlayState* play, int modelId, Actor* actor, float worldScale,
                                float groundOffset) {
    u8 tint[3];
...
typedef struct {
    ...
    Zelda3D_AnimResolver resolveAnim; // NULL = no live anim state (use `anim` + free frame)
    Zelda3D_JointResolver resolveJoints; // NULL = no N64-joint port (use CSAB path). Legacy
    ...
    int n64anim; // 1 = drive this model's OoT3D skeleton from the actor's LIVE N64 SkelAnime
                 // joints via the generic SkelAnime_Draw hook (Zelda3D_SkelAnimeDraw) when
                 // ZELDA3D_N64ANIM is on — no per-actor jointTable accessor needed. The OoT3D
...
static Zelda3D_ModelEntry sModelTable[] = {
...
static int Zelda3D_AutoMode(void) {
    if (gZelda3dAuto < 0) {
...
static void Zelda3D_EmitMeasure(PlayState* play, int key, int begin) {
    OPEN_DISPS(play->state.gfxCtx);
...
static int Zelda3D_ActorObjectId(PlayState* play, Actor* actor) {
...
static unsigned long long gZelda3dEnKoMaskOverride = ~0ull; // REPL `enkomask <hex>` (mid-ident sweep)
static int gZelda3dEnKoMaskOverrideSet = 0;
...
static int Zelda3D_SkyModelId(int idx) {
...
static void Zelda3D_FogSetPosition(float fmin, float fmax) {
...
static void Zelda3D_UpdateLight(PlayState* play) {
    EnvLightSettings* ls = &play->envCtx.lightSettings;
    float d[3];
    float len;
...
#include "zelda3d_player_animmap.inc"
