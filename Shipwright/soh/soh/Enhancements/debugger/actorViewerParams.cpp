...
constexpr s16 DEKUNUTS_FLOWER = 10;
...
bool ActorViewerActorUsesParams(u16 actorId) {
    return std::find(noParamsActors.begin(), noParamsActors.end(), actorId) == noParamsActors.end();
}

bool ActorViewerHasCustomParameterEditor(u16 actorId) {
    CreateActorSpecificData();
    return actorSpecificData.contains(actorId);
}

s16 ActorViewerDrawCustomParameterEditor(u16 actorId, s16 params) {
    CreateActorSpecificData();
    return actorSpecificData.at(actorId)(params);
}
