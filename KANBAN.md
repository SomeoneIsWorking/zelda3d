...
- [#205] N64 HUD is drawn by the Fast3D INTERPRETER — move it to a NATIVE draw path — user request 2026-07-28 (screenshot: item-button discs rendering as black bars; explicit instruction NOT to fix the glitch but to change the architecture). **Pass 1 landed; the card stays here because the conversion is not complete.** Architecture in place: the N64 HUD keeps its LAYOUT and only its final texrect emission becomes a `Zelda3D_HudQuad*` record, drawn in `Gui::EndFrame` by the restored SDL3-GPU quad renderer (extended with a per-vertex ENV colour + combine mode for the heart `(PRIM-ENV)*TEXEL0+ENV` case). **Converted: the item-button cluster** (disc + item icon + ammo count + key badge) — the region in the photo; its black bars are gone, and the shared-resident-tile + `bgScale` coupling that caused them no longer exists on that path. Evidence `scratch/screenshots/hud_native_after{,_zoom}.png` vs `hud_before_native.png`. **Pass 2 (2026-07-28) converted the A button + do-action label** — the black-bar stack (`docs/issues/0004-*`), identified by measurement: screen-anchored under `camorbit`, gone under REPL `hudtex 0`. These are not texrects but ortho-matrix quads, so placement was derived (`centre = (x+23, y+23)`) and checked against the game — native vs vanilla agree to 1.3 virtual units. The flip animation survives exactly (`cos` scale under ortho) and the IA4 label is decoded with a content-hash cache. **Pass 3 (2026-07-28)** converted the health meter, the magic meter and the rupee/small-key counters, and added REPL `nativehud 0|1` — a same-frame A/B, since comparing two separately-captured frames picks up scene motion behind the HUD. Expected residual between the paths is edge-only (linear sampler vs texrect filtering); filled-in diffs mean a real difference. **Pass 4 (2026-07-28)** converted the C-Up button + "Navi" label (its disc was corrupted on the interpreter path too — same shared-tile cause), the event timer (clock icon + digits) and the HBA score digits, and added REPL `navicall <0|1>` to hold the Navi prompt on (a one-shot poke never survives — `Interface_Update` clears the flag before the draw). **Still on the interpreter: the MINIMAP only** — and it must convert as a group with its compass/position/entrance icons, which draw over it, so a half-conversion would hide them. Elements must convert as a GROUP — the native pass runs after the whole interpreter frame, so a half-converted element inverts its own layering. Writeup `debug_journal/2026-07-28-hud-off-the-interpreter.md`. NOTE on "you can also use 3DS HUD": taken as permission, not a mandate — OoT3D's HUD is a DUAL-SCREEN design, so transplanting its layout to one screen is a redesign; say the word if that is wanted.
