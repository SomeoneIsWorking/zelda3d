...
- [#206] Link has NO SHADOW — user-reported 2026-07-28. **ROOT CAUSE FOUND (fix not yet written).** Link uses `ActorShadow_DrawFeet` (per-foot shadows), not the circle shadow, and it places them from `actor->shape.feetPos` — which is written ONLY by `Actor_SetFeetPos` from Link's post-limb-draw callback (`z_player_lib.c:2001`), i.e. during the **N64 skeleton limb walk**. Zelda3D replaces Link's draw with the OoT3D CMB model, so that walk never runs, `feetPos` is never written, and DrawFeet has nowhere to put a shadow. **Same pattern as the known collider bug** (memory `soh3d-skinned-actor-collision`: replaced draw skipped the limb walk → colliders at origin; fixed by re-running the walk for its side effects then REWINDING the display list so nothing draws). Apply that remedy — `feetPos` is another side effect of the same walk. **RULED OUT, do not re-chase:** shadow placement / terrain-warp Y offset — measured no-op (dark px under Link identical before/after), and `Zelda3D_TerrainWarpEnabled()` returns 0 whenever OoT3D collision is active anyway (`docs/issues/0006`). **Verify with a ground strip BELOW the boots** (px box ~320,395-450,435): a box containing Link is dominated by his dark boots and cannot see the shadow. Findings `docs/issues/0007`. Then continue with the other missing 3DS effects (user asked not to stop at the shadow); note anything beyond the N64 blob/feet shadow is a fresh RE+port — the Zelda3D sun-shadow/SSAO was deleted 2026-07-16.
