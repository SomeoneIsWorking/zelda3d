...
- [#206] Link has NO SHADOW — user-reported 2026-07-28. **ROOT CAUSE FOUND (not yet fixed).** It is NOT a missing system: `Actor_Draw` still calls `shape.shadowDraw` and `ActorShadow_Draw` is intact. The shadow is drawn at the **N64 COLLISION floor** — `func_80038A28(actor->floorPoly, world.pos.x, actor->floorHeight, world.pos.z, &mtx)` uses `actor->floorHeight` and never reads `world.pos.y` — while the visible ground is the OoT3D terrain, which sits at a different height. That gap is exactly what `Zelda3D_ActorRenderYOffset` exists to compensate for when drawing the MODEL (`z_actor.c` offsets `world.pos.y` for the draw and restores it right after, and the shadow draw is both outside that bracket AND immune to it). So the shadow renders buried in / floating off the OoT3D ground. **Fix direction:** place the shadow on the OoT3D render ground — `floorHeight + Zelda3D_ActorRenderYOffset(...)`, or the OoT3D ground height at that XZ (`Zelda3D_RenderYOffsetAtXZ`, memory `soh3d-terrain-warp`). Both of `ActorShadow_Draw`'s height-derived terms (the -50..500 visibility gate and the alpha/scale falloff from `world.pos.y - floorHeight`) stay correct as long as the two are shifted consistently. **Affects EVERY actor, not just Link** — verify on an NPC too. **Then continue with the other missing 3DS effects** (user asked explicitly not to stop at the shadow); note that memory `soh3d-title-no-loop-and-effects-removed` records the Zelda3D sun-shadow/SSAO enhancements as deliberately DELETED (2026-07-16, "OoT3D lighting only"), so anything beyond the actor blob shadow is a fresh RE+port, not a revival.
